...
export function useDashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        // Try Spring backend first
        const data = await dashboardService.getSummary();
        if (!cancelled) dispatch({ type: ACTIONS.DASHBOARD_SUCCESS, payload: data });
      } catch {
        ...
        if (!cancelled) {
          dispatch({
            ...
            payload: {
              stats: MOCK.stats,
              priorityIncident: MOCK.priorityIncident,
              escalations: MOCK.escalations,
              queues: MOCK.queues,
              activity: MOCK.activity,
              dailySummary: MOCK.dailySummary,
            },
          });
        }
      }
